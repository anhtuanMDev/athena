...
export default function DynamicSchemaNew() {
    ...
    async () => {
      try {
        const [schemas, enums] = await Promise.all([
          ...
          listDirectory(game!, "enums")
        ]);
        return { schemas, enums };
      } catch (e) {
        return { schemas: [], enums: [] as string[] };
      }
    },
    [game],
    "DynamicSchemaNew-49",
  ...
  const handleImportFields = () => {
    if (!importSchemaId || !loadedData?.schemas) return;
  ...
  return (
    <div className="w-full py-8 pb-32">
      <form onSubmit={handleCommit} className="space-y-8">
        ...
        {/* Schema Tools */}
        <div className="flex flex-wrap gap-2 items-center p-3 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-xl">
          ...
          <Button type="button" variant="outline" size="small" onClick={() => setShowPromptModal(true)} className="text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-700 hover:bg-blue-100 dark:hover:bg-blue-800/50">
            <Sparkles className="w-4 h-4 mr-1.5" /> Get AI Prompt
          </Button>
          <Button type="button" variant="outline" size="small" onClick={() => setShowImportModal(true)} className="text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-700 hover:bg-blue-100 dark:hover:bg-blue-800/50">
          ...
          <Button type="button" variant="outline" size="small" onClick={handleExportCopy} className="text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-700 hover:bg-emerald-100 dark:hover:bg-emerald-800/50">
            {exportCopied ? <Check className="w-4 h-4 mr-1.5 text-emerald-500" /> : <Copy className="w-4 h-4 mr-1.5" />}
            {exportCopied ? "Copied!" : "Copy JSON"}
          </Button>
          <Button type="button" variant="outline" size="small" onClick={handleExportDownload} className="text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-700 hover:bg-emerald-100 dark:hover:bg-emerald-800/50">
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-800 pb-4">
              ...
              <div className="flex gap-2">
                {loadedData?.schemas && loadedData.schemas.length > 0 && (
                        ...
                        {(field.type === "enum" || field.type === "list") && (
                          <div className="mb-3">
                            ...
                            >
                              ...
                              {loadedData?.enums?.map((eId: string) => (
          ...
          <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden">
              <div className="p-6 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center">
                ...
                <button type="button" onClick={() => setShowClearModal(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">✕</button>
              </div>
              <div className="p-6 space-y-4">
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  This will remove all <span className="font-semibold text-gray-900 dark:text-white">{fields.length} fields</span> from the schema. This action cannot be undone.
                </p>
                <div className="flex gap-3 justify-end">
                  <Button type="button" variant="outline" size="small" onClick={() => setShowClearModal(false)}>
                    Cancel
                  </Button>
                  <Button
                    type="button"
                    ...
                    onClick={() => { setFields([]); setShowClearModal(false); }}
                  >
