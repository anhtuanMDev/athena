...
export default function NewPatch() {
  ...
  const { data, loading, error } = useData(async () => {
    ...
    const patchSchemas = schemas.filter(s => s && s.category === "patch") as DynamicSchemaFile[];
      ...
      <div className="w-full py-8">
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-gray-500 mt-2 mb-4">You must create a schema for Patches before adding entries.</p>
        ...
        <CardContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {data.fields.map((f) => {
              ...
              return (
                <FormField key={f.key} name={f.key} label={f.label} required={f.required} type={type} />
              );
            })}
