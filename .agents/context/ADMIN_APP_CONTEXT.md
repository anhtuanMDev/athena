...
| Forms              | `react-hook-form` + Zod (`zodResolver`) or HTML Forms + Zod                                                                   | `react-hook-form` used where real-time `onChange` validation is needed (e.g., Enums, Heroes, Maps). Zod schemas double as form validation and data contract validation.                        |
| UI State Management| Custom `EmptyState` and `LoadErrorState` components                                                                         | Standardizes the presentation of 404/not-found and backend error states to ensure a premium, unified user experience across all modules.                                                       |
